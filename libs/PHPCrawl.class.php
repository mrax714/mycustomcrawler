...
class PHPWebSpider
{
  ...
  /**
   * The PHPWebSpiderHTTPRequest-Object
   *
   * @var PHPWebSpiderHTTPRequest
   */
  protected $PageRequest;
  
  /**
   * The PHPWebSpiderLinkCache-Object
   *
   * @var PHPWebSpiderURLCache
   */
  protected $LinkCache;
  
  /**
   * The PHPWebSpiderCookieCache-Object
   *
   * @var  PHPWebSpiderCookieCache
   */
   ...
   *
   * @var PHPWebSpiderRobotsTxtParser
   */
   ...
   *
   * @var PHPWebSpiderUserSendDataCache
   */
   ...
   *
   * @var int One of the PHPWebSpiderAbortReasons::ABORTREASON-constants.
   */
   ...
   * URl cache-type.
   *
   * @var int One of the PHPWebSpiderUrlCacheTypes::URLCACHE..-constants.
   */
  ...
  /**
   * PHPWebSpiderProcessCommunication-object
   *
   * @var PHPWebSpiderProcessHandler
   */
  protected $ProcessHandler = null;
  
  /**
   * PHPWebSpiderStatusHandler-object
   *
   * @var PHPWebSpiderStatusHandler
   */
   ...
   *
   * @var int One of the PHPWebSpiderMultiProcessModes-constants
   */
   ...
   *
   * @var PHPWebSpiderDocumentInfoQueue
   */
  ...
  { 
    ...
    // Utils-classes
    if (!class_exists("PHPWebSpiderUtils")) include_once($classpath."/Utils/PHPWebSpiderUtils.class.php");
    if (!class_exists("PHPWebSpiderEncodingUtils")) include_once($classpath."/Utils/PHPWebSpiderEncodingUtils.class.php");
    
    // URL-Cache-classes
    if (!class_exists("PHPWebSpiderURLCacheBase")) include_once($classpath."/UrlCache/PHPWebSpiderURLCacheBase.class.php");
    if (!class_exists("PHPWebSpiderMemoryURLCache")) include_once($classpath."/UrlCache/PHPWebSpiderMemoryURLCache.class.php");
    if (!class_exists("PHPWebSpiderSQLiteURLCache")) include_once($classpath."/UrlCache/PHPWebSpiderSQLiteURLCache.class.php");
    
    // PageRequest-class
    if (!class_exists("PHPWebSpiderHTTPRequest")) include_once($classpath."/PHPWebSpiderHTTPRequest.class.php");
    $this->PageRequest = new PHPWebSpiderHTTPRequest();
    $this->PageRequest->setHeaderCheckCallbackFunction($this, "handleHeaderInfo");
      
    // Cookie-Cache-class
    if (!class_exists("PHPWebSpiderCookieCacheBase")) include_once($classpath."/CookieCache/PHPWebSpiderCookieCacheBase.class.php");
    if (!class_exists("PHPWebSpiderMemoryCookieCache")) include_once($classpath."/CookieCache/PHPWebSpiderMemoryCookieCache.class.php");
    if (!class_exists("PHPWebSpiderSQLiteCookieCache")) include_once($classpath."/CookieCache/PHPWebSpiderSQLiteCookieCache.class.php");
    
    // URL-filter-class
    if (!class_exists("PHPWebSpiderURLFilter")) include_once($classpath."/PHPWebSpiderURLFilter.class.php");
    $this->UrlFilter = new PHPWebSpiderURLFilter();
    
    // RobotsTxtParser-class
    if (!class_exists("PHPWebSpiderRobotsTxtParser")) include_once($classpath."/PHPWebSpiderRobotsTxtParser.class.php");
    $this->RobotsTxtParser = new PHPWebSpiderRobotsTxtParser();
    
    // ProcessReport-class
    if (!class_exists("PHPWebSpiderProcessReport")) include_once($classpath."/PHPWebSpiderProcessReport.class.php");
    
    // UserSendDataCache-class
    if (!class_exists("PHPWebSpiderUserSendDataCache")) include_once($classpath."/PHPWebSpiderUserSendDataCache.class.php");
    $this->UserSendDataCache = new PHPWebSpiderUserSendDataCache();
    
    // URLDescriptor-class
    if (!class_exists("PHPWebSpiderURLDescriptor")) include_once($classpath."/PHPWebSpiderURLDescriptor.class.php");
    
    // PageInfo-class
    if (!class_exists("PHPWebSpiderDocumentInfo")) include_once($classpath."/PHPWebSpiderDocumentInfo.class.php");
    
    // Benchmark-class
    if (!class_exists("PHPWebSpiderBenchmark")) include_once($classpath."/PHPWebSpiderBenchmark.class.php");
    
    // URLDescriptor-class
    if (!class_exists("PHPWebSpiderUrlPartsDescriptor")) include_once($classpath."/PHPWebSpiderUrlPartsDescriptor.class.php");
    
    // CrawlerStatus-class
    if (!class_exists("PHPWebSpiderStatus")) include_once($classpath."/PHPWebSpiderStatus.class.php");
    
    // AbortReasons-class
    if (!class_exists("PHPWebSpiderAbortReasons")) include_once($classpath."/Enums/PHPWebSpiderAbortReasons.class.php");
    
    // RequestErrors-class
    if (!class_exists("PHPWebSpiderRequestErrors")) include_once($classpath."/Enums/PHPWebSpiderRequestErrors.class.php");
    
    // PHPWebSpiderUrlCacheTypes-class
    if (!class_exists("PHPWebSpiderUrlCacheTypes")) include_once($classpath."/Enums/PHPWebSpiderUrlCacheTypes.class.php");
    
    // PHPWebSpiderMultiProcessModes-class
    if (!class_exists("PHPWebSpiderMultiProcessModes")) include_once($classpath."/Enums/PHPWebSpiderMultiProcessModes.class.php");
    
    // PHPWebSpiderProcessHandler-class
    if (!class_exists("PHPWebSpiderProcessHandler")) include_once($classpath."/ProcessCommunication/PHPWebSpiderProcessHandler.class.php");
    
    // PHPWebSpiderStatusHandler-class
    if (!class_exists("PHPWebSpiderStatusHandler")) include_once($classpath."/ProcessCommunication/PHPWebSpiderStatusHandler.class.php");
    
    // PHPWebSpiderDocumentInfoQueue-class
    if (!class_exists("PHPWebSpiderDocumentInfoQueue")) include_once($classpath."/ProcessCommunication/PHPWebSpiderDocumentInfoQueue.class.php");
    
    // Set default temp-dir
    $this->working_base_directory = PHPWebSpiderUtils::getSystemTempDir();
  }
  ...
  protected function initCrawlerProcess()
  {
    ...
    // Setup url-cache
    if ($this->url_cache_type == PHPWebSpiderUrlCacheTypes::URLCACHE_SQLITE) 
      $this->LinkCache = new PHPWebSpiderSQLiteURLCache($this->working_directory."urlcache.db3", true);
    else
      $this->LinkCache = new PHPWebSpiderMemoryURLCache();
    
    // Perge/cleanup SQLite-urlcache for resumed crawling-processes (only ONCE!)
    if ($this->url_cache_type == PHPWebSpiderUrlCacheTypes::URLCACHE_SQLITE && $this->urlcache_purged == false)
    {
    ...
    if ($this->url_cache_type == PHPWebSpiderUrlCacheTypes::URLCACHE_SQLITE)
      $this->CookieCache = new PHPWebSpiderSQLiteCookieCache($this->working_directory."cookiecache.db3", true);
    else $this->CookieCache = new PHPWebSpiderMemoryCookieCache();
    
    // ProcessHandler
    $this->ProcessHandler = new PHPWebSpiderProcessHandler($this->crawler_uniqid, $this->working_directory);
    
    // Setup PHPWebSpiderStatusHandler
    $this->CrawlerStatusHandler = new PHPWebSpiderStatusHandler($this->crawler_uniqid, $this->working_directory);
    $this->setupCrawlerStatusHandler();
    
    // DocumentInfo-Queue
    if ($this->multiprocess_mode == PHPWebSpiderMultiProcessModes::MPMODE_PARENT_EXECUTES_USERCODE)
      $this->DocumentInfoQueue = new PHPWebSpiderDocumentInfoQueue($this->working_directory."doc_queue.db3", true);
    ...
    $url_descriptor = new PHPWebSpiderURLDescriptor($this->starting_url);
   ...
   * <ol>
   * <li><b>{@link PHPWebSpiderMultiProcessModes}::MPMODE_PARENT_EXECUTES_USERCODE</b>
   *
   ...
   * </li>
   * <li><b>{@link PHPWebSpiderMultiProcessModes}::MPMODE_CHILDS_EXECUTES_USERCODE</b>
   *
   ...
   * <code>
   * $crawler->goMultiProcessed(5, PHPWebSpiderMultiProcessModes::MPMODE_PARENT_EXECUTES_USERCODE);
   * </code>
   *
   ...
   *                               One of the {@link PHPWebSpiderMultiProcessModes}-constants
   * @section 1 Basic settings
   */
  ...
  { 
    ...
    PHPWebSpiderBenchmark::start("crawling_process");
    
    // Set url-cache-type to sqlite.
    $this->url_cache_type = PHPWebSpiderUrlCacheTypes::URLCACHE_SQLITE;
    ...
    PHPWebSpiderBenchmark::stop("crawling_process");
  }
  ...
  protected function startControllerProcessLoop()
  {
    ...
    if ($this->multiprocess_mode != PHPWebSpiderMultiProcessModes::MPMODE_PARENT_EXECUTES_USERCODE) exit;
    ...
    { 
      ...
      { 
        ...
        {
          $this->CrawlerStatusHandler->updateCrawlerStatus(null, PHPWebSpiderAbortReasons::ABORTREASON_PASSEDTHROUGH);
        }
      ...
      if ($user_abort == true) 
        $this->CrawlerStatusHandler->updateCrawlerStatus(null, PHPWebSpiderAbortReasons::ABORTREASON_USERABORT);
    }
  }
  ...
  { 
    ...
    {
      if ($this->multiprocess_mode == PHPWebSpiderMultiProcessModes::MPMODE_PARENT_EXECUTES_USERCODE) return;
      else exit;
    }
   ...
   *
   * @param PHPWebSpiderURLDescriptor $UrlDescriptor The URL as PHPWebSpiderURLDescriptor-object
   * @return bool TURE if the crawling-process should be aborted after processig the URL, otherwise FALSE.
   */
  protected function processUrl(PHPWebSpiderURLDescriptor $UrlDescriptor)
  { 
    // Check for abortion from other processes first if mode is MPMODE_CHILDS_EXECUTES_USERCODE
    if ($this->multiprocess_mode == PHPWebSpiderMultiProcessModes::MPMODE_CHILDS_EXECUTES_USERCODE)
    { 
      // Check for abortion (any limit reached?)
      if ($this->checkForAbort() !== null) return true;
    }
    
    PHPWebSpiderBenchmark::start("processing_url");
    ...
    PHPWebSpiderBenchmark::stop("processing_url");
    $PageInfo->benchmarks = PHPWebSpiderBenchmark::getAllBenchmarks();
    ...
    PHPWebSpiderBenchmark::resetAll(array("crawling_process"));
  ...
  {
    PHPWebSpiderBenchmark::start("processing_robots_txt");
    
    $robotstxt_rules = $this->RobotsTxtParser->parseRobotsTxt(new PHPWebSpiderURLDescriptor($this->starting_url),
                                                              $this->PageRequest->userAgentString,
                                                              $this->robots_txt_uri);
    $this->UrlFilter->addURLFilterRules($robotstxt_rules);
    
    PHPWebSpiderBenchmark::stop("processing_robots_txt");
  }
  
  /**
   * Checks if the crawling-process should be aborted.
   *
   * @return int NULL if the process shouldn't be aborted yet, otherwise one of the PHPWebSpiderAbortReasons::ABORTREASON-constants.
   */
  protected function checkForAbort()
  {
    PHPWebSpiderBenchmark::start("checkning_for_abort");
    ...
    if ($this->traffic_limit > 0 && $crawler_status->bytes_received >= $this->traffic_limit)
      $abort_reason = PHPWebSpiderAbortReasons::ABORTREASON_TRAFFICLIMIT_REACHED;
    ...
    if ($this->request_limit > 0)
    {
      ...
      {
        $abort_reason = PHPWebSpiderAbortReasons::ABORTREASON_FILELIMIT_REACHED;
      }
    ...
    PHPWebSpiderBenchmark::stop("checkning_for_abort");
  ...
  protected function delayRequest()
  {
    ...
    if ($this->request_delay_time != null)
    {
      while (true)
      {
        ...
        if ($crawler_status->last_request_time + $this->request_delay_time > PHPWebSpiderBenchmark::getmicrotime())
      ...
      $this->CrawlerStatusHandler->updateCrawlerStatus(null, null, null, PHPWebSpiderBenchmark::getmicrotime());
    }
  }
  ...
  protected function setupCrawlerStatusHandler()
  {
    ...
    if ($this->multiprocess_mode == PHPWebSpiderMultiProcessModes::MPMODE_CHILDS_EXECUTES_USERCODE || $this->resumtion_enabled == true)
    {
      $this->CrawlerStatusHandler->write_status_to_file = true;
    }
    
    if ($this->request_delay_time != null && $this->multiprocess_mode != PHPWebSpiderMultiProcessModes::MPMODE_NONE)
    {
      $this->CrawlerStatusHandler->write_status_to_file = true;
    }
  ...
  protected function cleanup()
  {
    ...
    // Delete working-dir
    PHPWebSpiderUtils::rmDir($this->working_directory);
    
    // Remove semaphore (if multiprocess-mode)
    if ($this->multiprocess_mode != PHPWebSpiderMultiProcessModes::MPMODE_NONE)
    {
   ...
   *
   * @return PHPWebSpiderProcessReport PHPWebSpiderProcessReport-object containing process-summary-information
   * @section 1 Basic settings
   */
  ...
  { 
    ...
    // Create report
    $Report = new PHPWebSpiderProcessReport();
    ...
    $Report->process_runtime = PHPWebSpiderBenchmark::getElapsedTime("crawling_process");
    ...
    if ($CrawlerStatus->abort_reason == PHPWebSpiderAbortReasons::ABORTREASON_TRAFFICLIMIT_REACHED)
      $Report->traffic_limit_reached = true;
    
    if ($CrawlerStatus->abort_reason == PHPWebSpiderAbortReasons::ABORTREASON_FILELIMIT_REACHED)
      $Report->file_limit_reached = true;
    
    if ($CrawlerStatus->abort_reason == PHPWebSpiderAbortReasons::ABORTREASON_USERABORT)
   ...
   *
   * For detailed information on the conatining array-keys see PHPWebSpiderProcessReport-class.
   * 
   ...
   * Example:
   * <code>
   * class MyCrawler extends PHPWebSpider 
   * {
   *   function handleHeaderInfo(PHPWebSpiderResponseHeader $header)
   *   {
   ...
   * </code>
   *
   * @param PHPWebSpiderResponseHeader $header The header as PHPWebSpiderResponseHeader-object
   ...
   */
  public function handleHeaderInfo(PHPWebSpiderResponseHeader $header)
  {
    return 1;
  }
   ...
   * process-context.
   *
   * So when using the multi-process mode "{@link PHPWebSpiderMultiProcessModes::MPMODE_CHILDS_EXECUTES_USERCODE}", this method
   ...
   * $crawler->goMultiProcessed(5, PHPWebSpiderMultiProcessModes::MPMODE_CHILDS_EXECUTES_USERCODE);
   * </code>
   *
   * @section 3 Overridable methods / User data-processing
   */
   ...
   * by a PHPWebSpiderDocumentInfo-object.
   *
   * Please see the {@link PHPWebSpiderDocumentInfo} documentation for a list of all properties describing the
   * html-document.
   *
   * Example:
   * <code>
   ...
   * </code>
   *
   * @param PHPWebSpiderDocumentInfo $PageInfo A PHPWebSpiderDocumentInfo-object containing all information about the currently received document.
   *                                         Please see the reference of the {@link PHPWebSpiderDocumentInfo}-class for detailed information.
   * @return int                             The crawling-process will stop immedeatly if you let this method return any negative value.
   *
   * @section 3 Overridable methods / User data-processing
   */
  public function handleDocumentInfo(PHPWebSpiderDocumentInfo $PageInfo){}
  ...
  public function setURL($url)
  {
    ...
    {
      $this->starting_url = PHPWebSpiderUtils::normalizeURL($url);
      return true;
    }
    else return false;
  }
  ...
  public function setPort($port)
  {
    ...
    $url_parts = PHPWebSpiderUtils::splitURL($this->starting_url);
    $url_parts["port"] = $port;
    $this->starting_url = PHPWebSpiderUtils::buildURLFromParts($url_parts, true);
  ...
  {
    $check = PHPWebSpiderUtils::checkRegexPattern($regex); // Check pattern
   ...
   * (see properties {@link PHPWebSpiderDocumentInfo::received_to_file} and {@link PHPWebSpiderDocumentInfo::content_tmp_file}).
   *
   ...
   *
   * The memory-cache ({@link PHPWebSpiderUrlCacheTypes}::URLCACHE_MEMORY) is recommended for spidering small to medium websites.
   * It provides better performance, but the php-memory-limit may be hit when too many URLs get added to the cache.
   * This is the default-setting.
   *
   * The SQlite-cache ({@link PHPWebSpiderUrlCacheTypes}::URLCACHE_SQLite) is recommended for spidering huge websites.
   ...
   * Example:
   * <code>
   * $crawler->setUrlCacheType(PHPWebSpiderUrlCacheTypes::URLCACHE_SQLITE);
   ...
   * to PHPWebSpiderUrlCacheTypes::URLCACHE_SQLITE.
   *
   * @param int $url_cache_type 1 -> in-memory-cache (default setting)
   *                            2 -> SQlite-database-cache
   *
   *                            Or one of the {@link PHPWebSpiderUrlCacheTypes}::URLCACHE..-constants.
   * @return bool
   * @section 1 Basic settings
   */
  ...
  public function enableResumption()
  {
    ...
    $this->setUrlCacheType(PHPWebSpiderUrlCacheTypes::URLCACHE_SQLITE);
  }
   ...
   * $crawler->setHTTPProtocolVersion(PHPWebSpiderHTTPProtocols::HTTP_1_1);
   * </code>
   *
   * Since phpcrawl 0.82, HTTP 1.1 is the default protocol.
   *
   * @param int $http_protocol_version One of the {@link PHPWebSpiderHTTPProtocols}-constants, or
   *                                   1 -> HTTP 1.0
   *                                   2 -> HTTP 1.1 (default)
   * @return bool
   * @section 1 Basic settings
   ...
   * to finding links.
   *
   * See {@link PHPWebSpiderLinkSearchDocumentSections}-constants for all predefined sections.
   *
   * Example 1:
   * <code>
   * // Let the crawler ignore script-sections and html-comment-sections when finding links
   * $crawler->excludeLinkSearchDocumentSections(PHPWebSpiderLinkSearchDocumentSections::SCRIPT_SECTIONS |
   *                                             PHPWebSpiderLinkSearchDocumentSections::HTML_COMMENT_SECTIONS);
   * </code>
   * Example 2:
   * <code>
   * // Let the crawler ignore all special sections except HTML-comments 
   * $crawler->excludeLinkSearchDocumentSections(PHPWebSpiderLinkSearchDocumentSections::ALL_SPECIAL_SECTIONS ^
   *                                             PHPWebSpiderLinkSearchDocumentSections::HTML_COMMENT_SECTIONS);
   * </code>
   *
   * @param int $document_sections Bitwise combination of the {@link PHPWebSpiderLinkSearchDocumentSections}-constants.
   * @section 6 Linkfinding settings 
   */
